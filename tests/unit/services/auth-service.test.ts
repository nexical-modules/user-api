...
vi.mock('@/lib/core/db', () => {
  const mockModelProps = { id: 'auth_test', name: 'Test' };
  ...
  (globalThis as unknown as Record<string, () => void>)._resetAuthServiceMocks =
    resetImplementations;
  return {
...
describe('AuthService', () => {
  beforeEach(() => {
    ...
    if (globalAny._resetAuthServiceMocks) {
      globalAny._resetAuthServiceMocks();
    }
  ...
  describe('register', () => {
    it('should run register successfully', async () => {
      const result = await (
        AuthService as unknown as Record<string, (...args: unknown[]) => unknown>
      ).register({ id: 'auth_test', name: 'Test' });
