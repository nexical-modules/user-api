...
vi.mock('@/lib/core/db', () => {
  const mockModelProps = { id: 'userAuth_test', name: 'Test' };
  ...
  (globalThis as unknown as Record<string, () => void>)._resetUserAuthServiceMocks =
    resetImplementations;
  return {
...
describe('UserAuthService', () => {
  beforeEach(() => {
    ...
    if (globalAny._resetUserAuthServiceMocks) {
      globalAny._resetUserAuthServiceMocks();
    }
  ...
  describe('validateToken', () => {
    it('should run validateToken successfully', async () => {
      const result = await (
        UserAuthService as unknown as Record<string, (...args: unknown[]) => unknown>
      ).validateToken('userAuth_test' as unknown);
