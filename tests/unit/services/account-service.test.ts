...
vi.mock('@/lib/core/db', () => {
  ...
  (globalThis as unknown as Record<string, () => void>)._resetAccountServiceMocks =
    resetImplementations;
  return {
...
describe('AccountService', () => {
  beforeEach(() => {
    ...
    if (globalAny._resetAccountServiceMocks) {
      globalAny._resetAccountServiceMocks();
    }
  ...
  describe('get', () => {
    ...
    it('should handle not found', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('account.service.error.not_found');
    });

    it('should handle errors when getting', async () => {
      vi.mocked(db.account.findUnique).mockRejectedValue(new Error('DB Error'));

      const result = await AccountService.get('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('account.service.error.get_failed');
    });
  });
  ...
  describe('create', () => {
    ...
    it('should handle errors when creating', async () => {
      vi.mocked(db.account.create).mockRejectedValue(new Error('DB Error'));

      const result = await AccountService.create({} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('account.service.error.create_failed');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should handle errors when updating', async () => {
      ...
      const result = await AccountService.update('1', {} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('account.service.error.update_failed');
    });
  });

  describe('delete', () => {
    it('should delete an Account', async () => {
      vi.mocked(db.account.delete).mockResolvedValue({} as unknown as Record<string, unknown>);
    ...
    it('should handle errors when deleting', async () => {
      vi.mocked(db.account.delete).mockRejectedValue(new Error('DB Error'));

      const result = await AccountService.delete('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('account.service.error.delete_failed');
    });
  });
});
vi.mock('@/lib/core/config', () => ({
