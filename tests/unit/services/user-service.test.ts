...
vi.mock('@/lib/core/db', () => {
  ...
  (globalThis as unknown as Record<string, () => void>)._resetUserServiceMocks =
    resetImplementations;
  return {
...
describe('UserService', () => {
  beforeEach(() => {
    ...
    if (globalAny._resetUserServiceMocks) {
      globalAny._resetUserServiceMocks();
    }
  ...
  describe('get', () => {
    ...
    it('should handle not found', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('user.service.error.not_found');
    });
    ...
    it('should handle errors when getting', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('user.service.error.get_failed');
    });
  });
  ...
  describe('create', () => {
    ...
    it('should handle errors when creating', async () => {
      ...
      const result = await UserService.create({} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('user.service.error.create_failed');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should handle errors when updating', async () => {
      ...
      const result = await UserService.update('1', {} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('user.service.error.update_failed');
    });
  });

  describe('delete', () => {
    it('should delete an User', async () => {
      vi.mocked(db.user.delete).mockResolvedValue({} as unknown as Record<string, unknown>);
    ...
    it('should handle errors when deleting', async () => {
      vi.mocked(db.user.delete).mockRejectedValue(new Error('DB Error'));

      const result = await UserService.delete('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('user.service.error.delete_failed');
    });
  });
});
