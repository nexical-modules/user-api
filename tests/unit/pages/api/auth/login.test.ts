...
vi.mock('../../../../../src/actions/login-auth');
vi.mock('@/lib/api/api-guard');

describe('Auth API - POST ../../../../../src/pages/api/auth/login', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call LoginAuthAction and return success', async () => {
    const query = ['GET', 'DELETE'].includes('POST'.toUpperCase())
      ? `?email=${encodeURIComponent(String('test@example.com'))}&password=${encodeURIComponent(String('test'))}`
      : '';
    ...
    vi.mocked(LoginAuthAction.run).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
