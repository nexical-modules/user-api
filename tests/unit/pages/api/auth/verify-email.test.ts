...
import { POST } from '../../../../../src/pages/api/auth/verify-email';
vi.mock('../../../../../src/actions/verify-email-auth');
vi.mock('@/lib/api/api-guard');

describe('Auth API - POST ../../../../../src/pages/api/auth/verify-email', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call VerifyEmailAuthAction and return success', async () => {
    ...
    vi.mocked(VerifyEmailAuthAction.run).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
