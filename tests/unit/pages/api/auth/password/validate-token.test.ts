...
import { POST } from '../../../../../../src/pages/api/auth/password/validate-token';
vi.mock('../../../../../../src/actions/validate-reset-token-auth');
vi.mock('@/lib/api/api-guard');

describe('Auth API - POST ../../../../../../src/pages/api/auth/password/validate-token', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call ValidateResetTokenAuthAction and return success', async () => {
    const query = ['GET', 'DELETE'].includes('POST'.toUpperCase())
      ? `?token=${encodeURIComponent(String('test-token'))}`
      : '';
    ...
    vi.mocked(ValidateResetTokenAuthAction.run).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
