...
import { AuthService } from '../../../src/actions/../services/auth-service';
...
vi.mock('../../../src/actions/../services/auth-service');

describe('RegisterAuthAction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should be defined', () => {
    expect(RegisterAuthAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const authServiceMock = vi.mocked(AuthService);
    commonServiceMethods.forEach((m) => {
      if ((authServiceMock as unknown as Record<string, vi.Mock>)[m])
        (authServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await RegisterAuthAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call AuthService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when AuthService fails', async () => {
    const serviceMock = vi.mocked(AuthService);
    commonServiceMethods.forEach((m) => {
