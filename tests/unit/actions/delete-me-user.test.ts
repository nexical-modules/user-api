...
import { UserService } from '../../../src/actions/../services/user-service';
...
vi.mock('../../../src/actions/../services/user-service');

describe('DeleteMeUserAction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should be defined', () => {
    expect(DeleteMeUserAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const userServiceMock = vi.mocked(UserService);
    commonServiceMethods.forEach((m) => {
      if ((userServiceMock as unknown as Record<string, vi.Mock>)[m])
        (userServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await DeleteMeUserAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call UserService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when UserService fails', async () => {
    const serviceMock = vi.mocked(UserService);
    commonServiceMethods.forEach((m) => {
